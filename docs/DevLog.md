...
## 2026-07-12 美术：地面/道路材质深化

- `fill_ground`：草地加大块明暗斑驳（`speckles` 椭圆色块）+ 双色草叶（深茎浅尖），告别平噪点。
- `draw_path`：土路加深色磨损边缘（`rim = _shade(base,-26)` 先铺再压浅中心）+ 石子 + 沿向车辙。
- 重生成青木村/黑狼林背景。asset/流程/HUD 测试通过。

## 2026-07-12 美术：星露谷风描边推广（主角/怪物/NPC/场景）

完成：

- 新增精灵规范 `docs/art/sprite_spec.md`：锁定尺寸/帧布局/原点/风格，保证「程序化（路径1）」与「后续手绘/AI 精修（路径2）」资源可无痛互换。
- 统一 **1px 深描边 + 阴影分离** 风格（`_finish_cell` / `_outline_cell`）：
  - 主角剑士行走/攻击 atlas 逐帧描边。
  - 野狼/黑狼头目逐帧描边（`_finish_wolf_cell`）。
- NPC 从多边形色块换为**带描边像素立绘**：新增 `_draw_villager` + `build_npcs`，生成村长（红袍持杖）/行商（绿衣背包）/铁匠（灰围裙持锤）；`npc.tscn` 与村长节点改用 `Sprite2D`，脚本按 `npc_id` 加载 `npc_%s.png`。
- 场景描边：`stamp_tree` / `stamp_hut` 增加轮廓层，树木与房屋获得 1px 深描边。

验证：全量断言测试 21/21 `PASS`；实机截图：主角、三 NPC、野狼、树木、房屋描边风格统一。

后续：进一步加强明暗层次与面部/装备细节；场景地面/道路材质精修。
